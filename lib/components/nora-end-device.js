...
NoraEndDeivce.prototype.readReq = function (path, callback) {
    ...
    Q.fcall(function () {
        ...
        // frmPayloadBuf = nutils.frmPayloadcrypto({ devAddr: self.info.devAddr, count: self.count }, 'down',frmPayloadBuf, self.info.appSKey);
        for (var i = 0;i < frmPayloadBuf.length;i += 1)
            buf.push(frmPayloadBuf[i]);
        // add mic
        buf = nutils.addMic(self, new Buffer(buf), 1);

        return buf;
    }).then(function (buf) {
        var data = new Buffer(buf);
        console.log('readReq data');
        console.log(data);
...
NoraEndDeivce.prototype.writeReq = function (path, value, callback) {
    ...
    return Q.fcall(function () {
        ...
        // encrypt frmPayload
        frmPayload = nutils.frmPayloadcrypto({ devAddr: self.info.devAddr, count: self.count }, 'down', frmPayload, self.appSKey);
