...
function Hal(spiCfg, rxIntPin, resetPin) {
    var self = this,
    ...
    //  this._reset : a mraa gpio of Reset
    ...
    resetPin = resetPin || defaultResetPin;
    ...
    else if (!_.isNumber(resetPin))
        throw new TypeError('resetPin should be a number if given');
    ...
    // set up the gpio for reset
    Object.defineProperty(this, '_reset', _.assign({
        // value: new m.Gpio(resetPin)
    }, propUnwritable));
...
Hal.prototype.config = function (settings, callback) {
    ...
    if (this._state === 'unknown') {
        ...
        deferred.reject(new Error('Chip modulation & frequency cannot be configured at run time'));
    }
...
Hal.prototype.reset = function (callback) {
    var self = this,
        deferred = Q.defer();
    // data spec, reset pin should be pulled low for 100 ms, & wait 5 ms before use chip
    this._reset.write(0);
    setTimeout(function () {
        self._reset.write(1);
        setTimeout(function () {
            self.config(self.radio, function (err, data) {
                return self.start().done(deferred.resolve, deferred.reject);
            });
        }, 10);
    }, 150);
    return deferred.promise.nodeify(callback);
...
Hal.prototype._initChip = function (settings, callback) {
    ...
    return this.write(REG.OPMODE, defaultMode).then(function () {
        ...
        // set interrupt trigger situation: rxDone 
        return self.write(REG.DIOMAP1, 0x00);
    }).nodeify(callback);
};
