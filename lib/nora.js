...
Nora.prototype._joinAccept = function (devEUI, config, appKey, callback) {
    ...
    if (dev)    // devAddr existed
        deferred.reject('End-device 0x' + settings.devAddr + ' does not exist.');
...
Nora.prototype.remove = function(devAddr, callback) {
    ...
    if (!end_device) {
        ...
        this._devBox.findFromDb({devAddr: devAddr}, function (err, dev) {
            self._devBox.remove(dev.id, function (err) {
                if (err)
                    deferred.reject(err);
                else {
...
Nora.prototype._parser = function (data, callback) {
    ...
    if (phyPayload.mhdr.mType === mType.JOINREQUEST) {      // join-request
        ...
        if (mic === Mic) {
            if (firstJoin)  // [TODO] after register, fire devIncoming
                self.emit('ind', {type: 'devIncoming', noraED: end_device});
            ...
            if (FHDR.fCtrl.fOptsLen === 0) {    // FOpts not existed
                if (FPort === null) {
                    ...
                    // decrypt FRMPayload
                    Payload = this._frmPayloadcrypto(device, Payload, device.nwkSKey);
                    ...
                    Payload = this._frmPayloadcrypto(device, Payload, device.appSKey);
                }
            ...
            // [TODO] delete end-device instance? if not first time join?
