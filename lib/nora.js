...
function Nora(config, options) {
    ...
    hal.on('data', function (data) {
        if (self._enabled) {
            var objPayload = nutils.parse(data);
            nutils.checkMicAndDecryptPayload(self, objPayload, function (err, data) {
                if (err)
                    throw new Error(err);
                else
                    nutils.divideEvent(self, data);
            });
        }
    });
}
...
/*** Protected APIs                                                                            ***/
/*************************************************************************************************/
Nora.prototype._autoRspMacCmd = function () {
