...
Nora.prototype.bind = function (abpInfo, config, callback) {
    var self = this,
        deferred = Q.defer(),
        end_device,
        settings = {
            ...
            devNonce: null,
            appKey: null,
            nwkSKey: abpInfo.nwkSKey,
            appSKey: abpInfo.appSKey,
            appNonce: config.appNonce,
            netId: config.netId,
            devAddr: abpInfo.devAddr,
            rx1DROffset: {},
            rx2DR: {},
            rxDelay: {}
    ...
    // [TODO] check specication, there may have other need to be done.
    this.find({devAddr: settings.devAddr, function (err, dev) {
        if (err)
            deferred.reject(err);
        else {
            if (dev === null) {
                ...
                deferred.resolve('Device already existed.');
            }
        }
...
Nora.prototype.activate = function (joinWay, config, callback) {
    ...
    // [TODO] check config, if there is any important information is not assigned, return error.
...
Nora.prototype.multicast = function (msg) {
...
Nora.prototype.joinAccept = function (devEUI, config, appKey, callback) {
    ...
    // MIC = cmac[0..3]
    // joinMic = joinMic.concat(mhdr.toString(16), settings.appEUI.toString(16), settings.devEUI.toString(16), settings.devNonce.toString(16));
    // joinMic = aesCmac(appKey, joinMic);
    // micCheck = joinMic.concat(joinMic[0], joinMic[1], joinMic[2], joinMic[3]);
    // if (micCheck === mic) { // MIC correct
        this.find({devAddr: settings.devAddr}, function (err, docs) {
            if (err)
                deferred.reject(err);
            else {
                // check devAddr
                if (docs === null) {  // devAddr does not exist
                    ...
                    deferred.resolve('Device address 0x' + settings.devAddr.toString(16) + ' existed.');
                }
            }
        });
    // } else {
    //     deferred.resolve('MIC is incorrect. Please check appkey.');
    // }
...
Nora.prototype.macReq = function(devAddr, cId, config, callback) {
    ...
    var buf = [],
        ...
        device;
    // check devAddr
    this.find({devAddr: devAddr}, function (err, dev) {
        if (err)
            deferred.reject(err);
        else {
            if (dev !== null) {
                ...
                device = undefined;
                deferred.resolve('Device address' + devAddr + 'not existed.');
            }
        }
        if (device === undefined)
            return deferred.promise.nodeify(callback);
    ...
    if (cId === 0x02) {         // linkCheckAns
        ...
        callback = config;
        // [TODO] linkCheckAns should be automatic response when end-device send linkCheckReq ?
        ...
        hal.read(REG.PKTSNRVALUE).then(function (pktSnr) {
            ...
            msg = msg.concat((0x49).toString(16), (0x00000000).toString(16), (0x01).toString(16), devAddr.toString(16), /*FHDR.FCnt.toString(16), */(0x00).toString(16), info.length.toString(16));
            // concat msg
            msg = msg.concat(mhdr.toString(16), devAddr.torString(16), fCtrl.toString(16), fCnt.toString(16));
            // FOpts content
            msg = msg.concat(margin.toString(16), this._gwCnt.toString(16));
            nwkSKey = device.nwkSKey;
            cmac = aesCmac(nwkSKey, msg);
            for (var i = 0;i < 4;i += 1)
                return buf.push(cmac[i]);
        }).then(function () {
            // [TODO] delay?
        ...
        msg = msg.concat(mhdr.toString(16), devAddr.torString(16), fCtrl.toString(16), fCnt.toString(16));
        // FOpts content
        msg = msg.concat(dataRate_Power.toString(16), ((data.chMask >> 8) && 0xff).toString(16), (data.chMask && 0xff).toString(16));
        nwkSKey = device.nwkSKey;
        cmac = aesCmac(nwkSKey, msg);
        for (var i = 0;i < 4;i += 1)
            buf.push(cmac[i]);
        ...
        msg = msg.concat(mhdr.toString(16), devAddr.torString(16), fCtrl.toString(16), fCnt.toString(16));
        // FOpts content
        msg = msg.concat((data.maxDCycle).toString(16));
        nwkSKey = device.nwkSKey;
        cmac = aesCmac(nwkSKey, msg);
        for (var i = 0;i < 4;i += 1)
            buf.push(cmac[i]);
        ...
        msg = msg.concat(mhdr.toString(16), devAddr.torString(16), fCtrl.toString(16), fCnt.toString(16));
        // FOpts content
        msg = msg.concat((data.dlSettings).toString(16), ((data.frequency >> 16) && 0xff).toString(16), ((data.frequency >> 8) && 0xff).toString(16), (data.frequency && 0xff).toString(16));
        nwkSKey = device.nwkSKey;
        cmac = aesCmac(nwkSKey, msg);
        for (var i = 0;i < 4;i += 1)
            buf.push(cmac[i]);
        ...
        msg = msg.concat(mhdr.toString(16), devAddr.torString(16), fCtrl.toString(16), fCnt.toString(16));
        // FOpts content
        msg = msg.concat(data.chIndex.toString(16), ((data.frequency >> 16) && 0xff).toString(16), ((data.frequency >> 8) && 0xff).toString(16), (data.frequency && 0xff).toString(16), data.drRange.toString(16));
        nwkSKey = device.nwkSKey;
        cmac = aesCmac(nwkSKey, msg);
        for (var i = 0;i < 4;i += 1)
            buf.push(cmac[i]);
        ...
        msg = msg.concat(mhdr.toString(16), devAddr.torString(16), fCtrl.toString(16), fCnt.toString(16));
        // FOpts content
        msg = msg.concat(data.delay.toString(16));
        nwkSKey = device.nwkSKey;
        cmac = aesCmac(nwkSKey, msg);
        for (var i = 0;i < 4;i += 1)
            buf.push(cmac[i]);
        ...
        msg = msg.concat(mhdr.toString(16), devAddr.torString(16), fCtrl.toString(16), fCnt.toString(16));
        // FOpts content
        msg = msg.concat(eirp_dwellTime.toString(16));
        nwkSKey = device.nwkSKey;
        cmac = aesCmac(nwkSKey, msg);
        for (var i = 0;i < 4;i += 1)
            buf.push(cmac[i]);
        ...
        msg = msg.concat(mhdr.toString(16), devAddr.torString(16), fCtrl.toString(16), fCnt.toString(16));
        // FOpts content
        msg = msg.concat(data.chIndex.toString(16), ((data.frequency >> 16) && 0xff).toString(16), ((data.frequency >> 8) && 0xff).toString(16), (data.frequency && 0xff).toString(16));
        nwkSKey = device.nwkSKey;
        cmac = aesCmac(nwkSKey, msg);
        for (var i = 0;i < 4;i += 1)
            buf.push(cmac[i]);
    ...
    } else if (cId === 0x10) {  // pingSlotInfoAns

    } else if (cId === 0x11) {  // pingSlotChannelReq

    } else if (cId === 0x12) {  // beaconTimingAns

    } else if (cId === 0x13) {  // beaconFreqReq
...
/*** Protected APIs                                                                            ***/
/*************************************************************************************************/
Nora.prototype._parser = function (data) {
    ...
    if (phyPayload.MHDR.MType === MType.JOINREQUEST) {      // join-request
        ...
        this._devBox.findFromDb({devNonce: joinInfo.devNonce, function (err, dev) {
            if (err)
                console.log(err);
            else {
                if (dev === null) {   // devNonce do not exist in database
                    var len = -1;
                    self._joinBox.find(function (joinData) {
                        // check devEUI & appEUI equal to joinBox's
                        if ((joinInfo.devEUI === joinData.devEUI) & (joinInfo.appEUI === joinData.appEUI)) {
                            ...
                            if (joinInfo.mic === micCheck) {    // MIC correct
                                // OTAA join-procedure
                                // [TODO] add devNonce to joinData
                                self.joinAccept(joinData.devEUI, joinData, joinData.appKey, function (err, device) {
        ...
        if (data.length - i - 4 > 1) {
            ...
            i += 1;
            phyPayload.MACPayload.Payload = data.readUIntBE(i, (data.length - i - 4));
            i += (data.length - i - 4);
        } else {
        ...
        phyPayload.MIC = data.readUIntBE(i, 4);
        i += 4;

        this._payloadCheck(devData, function (err, data) {
...
Nora.prototype._payloadCheck = function (info, callback) {
    ...
    this._joinBox.find(function (joinData) {
        if (joinData.devAddr === FHDR.DevAddr) {
            ...
            len = self._joinBox.indexOf(joinData);
            // [TODO] delete this joinData content
        } else {
            // search devAddr in database(objectbox)
            this.find({devAddr: FHDR.DevAddr}, function (err, dev) {
                if (err)
                    deferred.reject(err);
                else {
                    if (dev === null) { // device address is not find
                        device = undefined;
                    } else {
                        device = dev;
                    }
    ...
    if (device !== undefined) {
        ...
        // concat msg
        msg = msg.concat(info.MHDR.toString(16), FHDR.devAddr.torString(16), FHDR.FCtrl.toString(16), FHDR.FCnt.toString(16));
        if (FHDR.FCtrl.FOptsLen !== 0)
        ...
        nwkSKey = device.nwkSKey;
        cmac = aesCmac(nwkSKey, msg);
        mic = mic.concat(cmac[0], cmac[1], cmac[2], cmac[3]);
    } else
        mic = null;
    ...
    if (mic === payloadMic) {
        ...
        if (FHDR.FCtrl.FOptsLen === 0) {    // FOpts not existed
            if (FPort !== null) {
                ...
                // decrypt FRMPayload
                Payload = frmPayloadcrypto(device, Payload, key);
            } else {
                // empty payload
            ...
            if (FHDR.FOpts[0] === 0x02)    // automatic response linkCheckAns(cId:0x02)
                this.macReq(FHDR.DevAddr, 0x02);
            // [TODO] Class B MAC command
        }
...
function frmPayloadcrypto (device, data, key) {
    if (!_.isBuffer(data))
    ...
    for (var i = 1;i < (data.length / 16);i += 1) {
        seq = buf.concat((0x01).torString(16), (0x00000000).toString(16), (0x00).toString(16), device.devAddr.toString(16), device.FCnt/*FCnt(Up/Down)*/, (0x00)toString(16), i.toString(16));
        buf = '';
        cipher.update(seq);
        seqSum = cipher.final();
        seqSum = seqSum.concat(seq);
    }
