...
Nora.prototype.multicast = function (data, callback) {
    ...
    if (nowTime === this._nextBeaconTime) {
        ...
        payload = nutils.frmPayloadcrypto(device, 'downlink', data, device.multicastKey);
...
/*** Protected APIs                                                                            ***/
/*************************************************************************************************/
Nora.prototype.sendMessage = function (devAddr, confirm, ack, pendingBit, cmdId, payload, callback) {
    if (!_.isBoolean(confirm))
        throw new TypeError('confirm should be a boolean.');
    if (!_.isBoolean(ack))
        throw new TypeError('ack should be a boolean.');
    if (!_.isBoolean(pendingBit))
        throw new TypeError('pendingBit should be a boolean.');
    if (!_.isNumber(cmdId))
        throw new TypeError('cmdId should be a number.');
    ...
    return Q.fcall(function () {
        ...
        // fctrl
        ack = ack ? 1 : 0;
        pendingBit = pendingBit ? 1 : 0;
        buf.push((ack << 5) | (pendingBit << 4));
        return buf;
    }).then(function (buf) {
        ...
        // console.log('sendMessage6');
        // frmPayload
        // encrypted payload
        payload = nutils.frmPayloadcrypto(noraED, 'downlink', payload, noraED.info.appSKey);
        ...
        // generate mic
        buf = nutils.addMic(noraED, new Buffer(buf), 'downlink');
        return buf;
    }).then(function (buf) {
