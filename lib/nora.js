...
function Nora(config, options) {
    ...
    // class B multicast address
    this.multicastAddr = option.multicastAddr;
    // class B multicast key
    this.multicastKey = option.multicastKey;
    ...
    // [TODO] why use bind, not use prototype?
...
Nora.prototype.activate = function (joinWay, config, callback) {
    ...
    if (joinWay === 'ABP') {
        if (devAddr === undefined | nwkSKey === undefined | appSKey === undefined | netId === undefined)
            throw new Error('devAddr, nwkSKey, appSKey or netId can not be undefined, please assign value to those parameters.');
        // push ABP config
        this._joinBox.push(config);
    } else if (joinWay === 'OTAA') {
        if (appEUI === undefined | devEUI === undefined | appKey === undefined | netId === undefined | devAddr === undefined | appNonce === undefined)
            throw new Error('appEUI, devEUI, appKey, netId, devAddr or appNonce can not be undefined, please assign value to those parameters.');
        // push OTAA conifg
        this._joinBox.push(config);
    }
};
...
Nora.prototype.joinAccept = function (devEUI, config, appKey, callback) {
    ...
    this.find({devAddr: settings.devAddr}, function (err, dev) {
        if (err)
            ...
            // devAddr
            if (dev === null) {  // devAddr does not exist
...
Nora.prototype._beacon = function (netId, callback) {
    ...
    // if (time < 0 || time > 128)
    //     throw new RangeError('time should be an integer in between 0 and 128');
    ...
    if (this._bOpt === true) {
        ...
        for (var i = 2;i > -1;i -= 1)
            data.push(netId[i]);
        data.push(this._beaconTime & 0xff);
        data.push((this._beaconTime >> 8) & 0xff);
        data.push((this._beaconTime >> 16) & 0xff);
        ...
        // [TODO] rfu is used or not? if rfu used, rfu should be 0
...
Nora.prototype.info = function() {
    return {
        ...
        beaconTime: this._nextBeaconTime
    };
};
...
Nora.prototype.macReq = function(devAddr, cmdId, config, callback) {
    ...
    if (device === undefined)
        deferred.resolve('Device address ' + devAddr + ' do not existed.');
    else {
        // [TODO] all mac command should be here
    ...
    if (cmdId === 0x02) {
        ...
        fOpts.push(data.frequency & 0xff);
        fOpts.push((data.frequency >> 8) & 0xff);
        fOpts.push((data.frequency >> 16) & 0xff);
    ...
    if (phyPayload.MHDR.MType === MType.JOINREQUEST) {      // join-request
        ...
        this._devBox.findFromDb({devNonce: joinInfo.devNonce, function (err, dev) {
            if (err)
                ...
                if (dev === []) {   // devNonce do not exist in database
                    var len = -1;
...
Nora.prototype._payloadCheck = function (info, callback) {
    ...
    if (len > -1)
        this._joinBox.splice(len, 1);   // delete this joinData content
...
function permitJoin(time) {
    ...
    this._permitJoinCountDown = setInterval(function () {
        ...
        self.emit('permitJoining', this._permitJoinTime);
    }, 1000);
